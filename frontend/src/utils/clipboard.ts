/**
 * Cross-platform, cross-browser clipboard copy function.
 * Supports:
...
export async function copyToClipboard(text: string): Promise<boolean> {
  ...
  if (typeof document !== 'undefined') {
    try {
      ...
      // Prevent scrolling and viewport shifting on mobile
    ...
    // 3. Fallback using span range selection (especially for iOS Safari where readonly textarea can block execCommand)
    try {
